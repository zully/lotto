...
def index():
    ...
    return template('index', version=VER, selected='',
                    games=games, base=False, exn=False)
...
def give_nums():
    ...
    if game_type == 'lotto_texas':
        base = get_base_nums(54, 6)
        exn = False
        game_code = 'WLD1JCMNS'

    elif game_type == 'power_ball':
        base = get_base_nums(69, 5)
        exn = get_num(26)
        game_code = 'WPD1JCMNS'

    elif game_type == 'mega_millions':
        base = get_base_nums(70, 5)
        exn = get_num(25)
        game_code = 'WMD1JCMNS'

    elif game_type == 'two_step':
        base = get_base_nums(35, 4)
        exn = get_num(35)
        game_code = 'WSD1S'
